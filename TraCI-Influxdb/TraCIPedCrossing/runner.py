...
# Schema to insert data
point_schema = [
        {
            "measurement": "pedestrian",
            "tags": {
                "host": "server01",
                "region": "us-west"
            },
...
def checkWaitingPersons():
    ...
    for edge in WALKINGAREAS:
        ...
        for ped in peds:
            timestamp = datetime.now() - timedelta(hours=1) # This is done to make it UTC
            ...
            if (traci.person.getWaitingTime(ped) == 1 and
                ...
                print("Current time: %s -> %s: pedestrian %s pushes the button (waiting: %s)" %
                      (timestamp, time, ped, numWaiting))
                waiting = True

                # Get current point schema
                point = point_schema

                point[0]['time'] = timestamp
                point[0]['fields']['num_pers'] = numWaiting
                point[0]['fields']['id'] = ped
